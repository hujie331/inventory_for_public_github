...
dev_grp = '/home/jhu/PycharmProjects/inventory/inventory_lab1a1b_pan.json'
...
for device in devices:
    ...
    url = f"https://{device['ip address']}/api/?type=op&cmd=<show><config><running></running></config></show>&"
    # response = requests.request("GET", url, headers=headers, data=payload
    response = requests.request("GET", url, verify=False, headers=headers,
                                data=payload)  # 'verify=False,' is mandatory,
    # otherwise the code will be interrupted when running.
